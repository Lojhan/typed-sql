...
import {
  ...
  type CompoundSelect,
  type ConflictAlgorithm,
  type ConflictTargetTerm,
...
class Parser {
  ...
  #parseSelect(withClause?: WithClause): SelectStatement {
    ...
    if (this.#current().value === "LOCK") {
      throw this.#error("LOCK IN SHARE MODE is MySQL syntax", this.#current().range);
    }
    ...
    if (["UNION", "INTERSECT", "EXCEPT"].includes(this.#current().value)) {
      ...
      if (all && operatorToken.value !== "UNION") {
      ...
      const flattened = this.#flattenCompoundStatement(statement);
      orderBy = [...trailingOrderBy];
      limit = trailingLimit;
      offset = trailingOffset;
      ...
      compounds.push({
        ...
        range: mergeRanges(operatorToken.range, flattened.statement.range),
      });
      compounds.push(...flattened.compounds);
    }
  ...
  #flattenCompoundStatement(statement: SelectStatement): {
    readonly statement: SelectStatement;
    readonly compounds: readonly CompoundSelect[];
  } {
    const { compounds, ...head } = statement;
    const flattened: CompoundSelect[] = [];
    for (const compound of compounds) {
      const nested = this.#flattenCompoundStatement(compound.statement);
      flattened.push({ ...compound, statement: nested.statement }, ...nested.compounds);
    }
    return { statement: { ...head, compounds: [] }, compounds: flattened };
  }
  ...
  #parseSelectLockingClause(): SelectLockingClause {
    ...
    if (this.#matchKeyword("UPDATE")) strength = "update";
      ...
      throw this.#error("FOR NO KEY UPDATE is PostgreSQL syntax", start);
    } else if (this.#matchKeyword("KEY")) {
      throw this.#error("FOR KEY SHARE is PostgreSQL syntax", start);
    } else {
  ...
  #parseInsert(withClause?: WithClause): InsertStatement {
    const start = withClause?.range ?? this.#current().range;
    const replace = this.#matchKeyword("REPLACE");
    if (!replace) this.#expectKeyword("INSERT");
    let conflictAlgorithm: ConflictAlgorithm | undefined = replace ? "replace" : undefined;
    if (!replace && this.#matchKeyword("OR")) conflictAlgorithm = this.#parseConflictAlgorithm();
    ...
    const upserts = [];
    while (this.#current().value === "ON") upserts.push(this.#parseUpsert());
    const returning = this.#matchKeyword("RETURNING") ? this.#parseSelectList() : [];
    return {
      kind: "insert",
      ...(withClause === undefined ? {} : { with: withClause }),
      table,
      ...(conflictAlgorithm === undefined ? {} : { conflictAlgorithm }),
      columns,
      source,
      upserts,
      returning,
  ...
  #parseUpdate(withClause?: WithClause): UpdateStatement {
    ...
    const conflictAlgorithm = this.#matchKeyword("OR") ? this.#parseConflictAlgorithm() : undefined;
    const table = this.#parseNamedTableReference(true);
  ...
  #parseConflictAlgorithm(): ConflictAlgorithm {
    const algorithm = this.#current().value.toLowerCase();
    if (!["rollback", "abort", "fail", "ignore", "replace"].includes(algorithm))
      throw this.#error("Expected ROLLBACK, ABORT, FAIL, IGNORE, or REPLACE after OR", this.#current().range);
    this.#advance();
    return algorithm as ConflictAlgorithm;
  }

  #parseUpsert(): InsertStatement["upserts"][number] {
    const start = this.#expectKeyword("ON").range;
    this.#expectKeyword("CONFLICT");
    const target: ConflictTargetTerm[] = [];
    let targetWhere: Expression | undefined;
    if (this.#matchPunctuation("(")) {
      do {
        const expression = this.#parseExpression();
        let collation: Identifier | undefined;
        if (this.#matchKeyword("COLLATE")) collation = this.#parseIdentifier(true);
      ...
      this.#expectPunctuation(")");
      if (this.#matchKeyword("WHERE")) targetWhere = this.#parseExpression();
    }
    this.#expectKeyword("DO");
    if (this.#matchKeyword("NOTHING")) {
      const range = mergeRanges(start, this.#previous().range);
      return {
        target,
        ...(targetWhere === undefined ? {} : { targetWhere }),
        action: { kind: "nothing", range },
        range,
      };
    }
    this.#expectKeyword("UPDATE");
    if (target.length === 0) throw this.#error("DO UPDATE requires a conflict target", this.#previous().range);
    this.#expectKeyword("SET");
    const assignments: UpdateAssignment[] = [];
    do {
      const column = this.#parseIdentifier();
    ...
    const where = this.#matchKeyword("WHERE") ? this.#parseExpression() : undefined;
    const actionRange = mergeRanges(start, where?.range ?? assignments.at(-1)?.range ?? this.#previous().range);
    return {
      target,
      ...(targetWhere === undefined ? {} : { targetWhere }),
      action: {
  ...
  #parseTableReference(): TableReference {
    ...
    if (this.#matchPunctuation("(")) {
      const args = this.#matchPunctuation(")") ? [] : [...this.#parseExpressionList()];
      const close = this.#previous().value === ")" ? this.#previous() : this.#expectPunctuation(")");
      let alias: Identifier | undefined;
      if (this.#matchKeyword("AS")) alias = this.#parseIdentifier();
      else if (this.#current().kind === "identifier" || this.#current().kind === "quoted-identifier")
      ...
      return {
        ...
        ...(alias === undefined ? {} : { alias }),
        lateral,
        range: mergeRanges(start, alias?.range ?? close.range),
      };
    }
    return this.#finishNamedTableReference(schema, tableName, true, lateral, start);
  }
  ...
  #parseNamedTableReference(allowAlias: boolean, lateral = false, start = this.#current().range): NamedTableReference {
    ...
    return this.#finishNamedTableReference(schema, name, allowAlias, lateral, start);
  }

  #finishNamedTableReference(
    schema: Identifier | undefined,
    name: Identifier,
    allowAlias: boolean,
    lateral: boolean,
    start: SourceRange,
    ...
    return this.#nested(() => {
      ...
      while (true) {
        ...
        if (special === "IN") {
          ...
          let values: InExpression["values"];
          let end: SourceRange;
          if (this.#current().value !== "(") {
            const table = this.#parseTableReference();
            if (table.kind === "subquery") throw this.#error("IN subquery must be parenthesized", table.range);
            values = table;
            end = table.range;
          } else {
            this.#expectPunctuation("(");
          ...
          left = { kind: "in", expression: left, values, negated, range: mergeRanges(start, end) };
          continue;
        }
        if (special === "BETWEEN") {
          const strength = 3;
  ...
  #parseWindowSpecification(): WindowSpecification {
    ...
    if (
      ...
      !["PARTITION", "ORDER", "ROWS", "RANGE", "GROUPS"].includes(this.#current().value)
    ) {
    ...
    if (["ROWS", "RANGE", "GROUPS"].includes(this.#current().value)) frame = this.#parseWindowFrame();
    const close = this.#expectPunctuation(")");
    return {
      ...(base === undefined ? {} : { base }),
      partitionBy,
  ...
  #parseWindowFrame(): NonNullable<WindowSpecification["frame"]> {
    const unitToken = this.#current();
    this.#advance();
    const unit = unitToken.value.toLowerCase() as NonNullable<WindowSpecification["frame"]>["unit"];
    let start: WindowFrameBoundary;
    let end: WindowFrameBoundary;
    if (this.#matchKeyword("BETWEEN")) {
      start = this.#parseWindowBoundary();
      this.#expectKeyword("AND");
      end = this.#parseWindowBoundary();
    } else {
      start = this.#parseWindowBoundary();
      end = { kind: "current-row", range: start.range };
    }
    if (start.kind === "unbounded-following")
      throw this.#error("A window frame cannot start with UNBOUNDED FOLLOWING", start.range);
    if (end.kind === "unbounded-preceding")
      throw this.#error("A window frame cannot end with UNBOUNDED PRECEDING", end.range);
    const boundaryOrder = {
    ...
    if (boundaryOrder[end.kind] < boundaryOrder[start.kind])
      throw this.#error("A window frame end cannot precede its start", end.range);
    let exclude: NonNullable<WindowSpecification["frame"]>["exclude"] = "no-others";
    if (this.#matchKeyword("EXCLUDE")) {
      if (this.#matchKeyword("NO")) {
        ...
        exclude = "current-row";
      } else if (this.#matchKeyword("GROUP")) exclude = "group";
      else {
        this.#expectKeyword("TIES");
        exclude = "ties";
      }
    }
    return { unit, start, end, exclude, range: mergeRanges(unitToken.range, this.#previous().range) };
  }

  #parseWindowBoundary(): WindowFrameBoundary {
    const start = this.#current().range;
    ...
    if (this.#matchKeyword("CURRENT")) {
      this.#expectKeyword("ROW");
      return { kind: "current-row", range: mergeRanges(start, this.#previous().range) };
    }
    const expression = this.#parseExpression();
    if (this.#matchKeyword("PRECEDING"))
  ...
  #isStatementStart(): boolean {
    return ["SELECT", "INSERT", "REPLACE", "UPDATE", "DELETE", "WITH"].includes(this.#current().value);
  }
