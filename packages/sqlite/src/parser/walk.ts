  ...
  switch (expression.kind) {
    ...
    case "call":
      ...
      if (expression.over !== undefined && "partitionBy" in expression.over) {
        ...
        if (expression.over.frame?.start.kind === "preceding" || expression.over.frame?.start.kind === "following")
          walkExpression(expression.over.frame.start.expression, statement, visitor, context);
        if (expression.over.frame?.end.kind === "preceding" || expression.over.frame?.end.kind === "following")
          walkExpression(expression.over.frame.end.expression, statement, visitor, context);
      }
      break;
    case "cast":
    ...
    case "in":
      ...
      if ("kind" in expression.values) {
        if (expression.values.kind === "select") walkStatementWithContext(expression.values, visitor, context.ctes);
        else walkTable(expression.values, statement, visitor, context);
...
function walkTable(table: TableReference, statement: Statement, visitor: SqlAstVisitor, context: SqlAstContext): void {
  ...
  if (table.kind === "table-function")
    for (const argument of table.arguments) walkExpression(argument, statement, visitor, context);
}
...
function walkStatementWithContext(
  ...
  if (statement.kind === "select") {
    ...
    for (const window of statement.windows) {
      ...
      if (
        window.specification.frame?.start.kind === "preceding" ||
        window.specification.frame?.start.kind === "following"
      )
        walkExpression(window.specification.frame.start.expression, statement, visitor, context);
      if (window.specification.frame?.end.kind === "preceding" || window.specification.frame?.end.kind === "following")
        walkExpression(window.specification.frame.end.expression, statement, visitor, context);
    }
  ...
  if (statement.kind === "insert") {
    ...
    for (const upsert of statement.upserts) {
      for (const target of upsert.target) walkExpression(target.expression, statement, visitor, context);
      if (upsert.targetWhere !== undefined) walkExpression(upsert.targetWhere, statement, visitor, context);
      if (upsert.action.kind === "update") {
        for (const assignment of upsert.action.assignments)
          walkExpression(assignment.value, statement, visitor, context);
        if (upsert.action.where !== undefined) walkExpression(upsert.action.where, statement, visitor, context);
      }
    }
