...
await describe("PostgreSQL runtime adapter", async () => {
  await it("uses policy-aware result codecs", () => {
    ...
    const values = createPostgresTypeParsers({ bigint: "bigint", numeric: "Decimal", date: "Date", json: "unknown" }, (value) => ({ decimal: value }));
    ...
    strict.deepStrictEqual(values.getTypeParser(1016)('{{1,2},{3,NULL}}'), [[1n, 2n], [3n, null]]);
    strict.deepStrictEqual(values.getTypeParser(199)('{"{\\\"ok\\\":true}",NULL}'), [{ ok: true }, null]);
    strict.throws(() => createPostgresTypeParsers({ bigint: "bigint", numeric: "Decimal", date: "Date", json: "unknown" }), /requires a decimal/);
  ...
  await it("rolls back nested savepoints and preserves outer rollback errors", async () => {
    ...
    await strict.rejects(
      () => nestedDb.transaction(async (transaction) => {
        await transaction.transaction(async () => { throw new Error("nested failed"); });
      }),
      /nested failed/,
    );
