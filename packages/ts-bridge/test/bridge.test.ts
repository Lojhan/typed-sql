...
await describe("TypeScript 7 editor bridge", async () => {
  ...
  await it("creates an editor-neutral inferred query and binding range", () => {
    ...
    strict.ok(analysis.transformedSource.includes('sql<{ "id": number; "name": string; "age": bigint | null; }, readonly []>`'));
  ...
  await it("maps cumulative append fragment overlays without producing partial-query diagnostics", () => {
    ...
    strict.ok(composed.insertions.every((insertion, index, values) => index === 0 || insertion.position >= (values[index - 1]?.position ?? 0)));
  });
  ...
  await it("maps conditional structural fragment overlays as complete SQL variants", () => {
    ...
    strict.ok(structural.transformedSource.includes('sql.withRow<Select["age"] extends true'));
