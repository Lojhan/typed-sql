...
const numericTypes = new Set(["tinyint", "smallint", "mediumint", "int", "integer", "bigint", "decimal", "numeric", "float", "double", "real", "bit", "year"]);
const comparisonOperators = new Set(["=", "!=", "<>", "<", "<=", ">", ">=", "IS", "IS NOT", "IS DISTINCT FROM", "IS NOT DISTINCT FROM", "LIKE", "NOT LIKE", "AND", "OR"]);
...
function normalized(databaseType: string): string {
  return databaseType.trim().toLowerCase().replace(/\s+unsigned$/u, "").replace(/\(.*/u, "");
}

function union(types: readonly string[]): string {
  const values = [...new Set(types)];
  return values.includes("unknown") ? "unknown" : values.join(" | ") || "unknown";
}
...
class Resolver {
  ...
  #statement(statement: Statement, outer: Scope | undefined, inherited: ReadonlyMap<string, TableSnapshot>): Omit<ResolvedMySqlQuery, "diagnostics" | "parameters"> {
    ...
    if (statement.kind === "insert") {
      const targets = statement.columns.length === 0
      ...
      if (statement.source.kind === "values") {
        ...
        if (selected.columns.length !== targets.length) this.#diagnostic("TSQ214", `INSERT has ${targets.length} target columns but SELECT returns ${selected.columns.length}`, statement.source.range);
      }
      if (statement.returning.length > 0) this.#unsupported("MySQL does not support INSERT RETURNING", statement.returning[0]!.range);
  ...
  #with(withClause: WithClause | undefined, outer: Scope | undefined, inherited: ReadonlyMap<string, TableSnapshot>): Map<string, TableSnapshot> {
    ...
    for (const query of withClause.queries) {
      ...
      if (result.resultKind === "command") this.#diagnostic("TSQ212", `CTE ${query.name.name} does not return rows`, query.range);
      if (query.columns.length > 0 && query.columns.length !== result.columns.length) this.#diagnostic("TSQ213", `CTE ${query.name.name} column count does not match its query`, query.range);
      const columns: Record<string, ColumnSnapshot> = {};
      result.columns.forEach((column, index) => {
        const columnName = query.columns[index] === undefined ? column.name : name(query.columns[index]!);
        columns[columnName] = { name: columnName, databaseType: column.databaseType ?? "unknown", tsType: column.tsType, nullable: column.nullable };
      });
  ...
  #select(statement: SelectStatement, outer: Scope | undefined, ctes: ReadonlyMap<string, TableSnapshot>): readonly ResolvedMySqlColumn[] {
    const scope: Scope = { relations: [], usingColumns: new Map(), ...(outer === undefined ? {} : { outer }) };
  ...
  #join(join: SelectStatement["joins"][number], scope: Scope, ctes: ReadonlyMap<string, TableSnapshot>): void {
    ...
    for (const identifier of join.using) {
      ...
      scope.usingColumns.set(columnName, {
        tsType: union([left.tsType, rightType.tsType]),
  ...
  #relation(reference: TableReference, nullable: boolean, scope: Scope, ctes: ReadonlyMap<string, TableSnapshot>): Relation | undefined {
    ...
    if (reference.kind === "subquery") {
      ...
      const columns: Record<string, ColumnSnapshot> = {};
      for (const column of result.columns) columns[column.name] = { name: column.name, databaseType: column.databaseType ?? "unknown", tsType: column.tsType, nullable: column.nullable };
      ...
      if (table === undefined) {
        const matches = Object.entries(this.#schema.tables).filter(([key, candidate]) => {
          const tableMatches = key.toLowerCase() === requested.toLowerCase() || key.toLowerCase().endsWith(`.${requested.toLowerCase()}`) || candidate.name.toLowerCase() === requested.toLowerCase();
          return tableMatches && (requestedSchema === undefined || candidate.schema?.toLowerCase() === requestedSchema.toLowerCase() || key.toLowerCase() === `${requestedSchema}.${requested}`.toLowerCase());
        });
        if (matches.length === 0) {
        ...
        if (requestedSchema === undefined && matches.length > 1) {
          this.#diagnostic("TSQ107", `Ambiguous table ${reference.name.name}`, reference.name.range, "Qualify the table with a database name.");
          return undefined;
        }
        table = matches[0]![1];
      }
  ...
  #items(items: readonly SelectItem[], scope: Scope, ctes: ReadonlyMap<string, TableSnapshot>): readonly ResolvedMySqlColumn[] {
    ...
    const add = (column: ResolvedMySqlColumn): void => {
      ...
      else { names.add(column.name); columns.push(column); }
    };
    for (const item of items) {
      if (item.expression.kind === "star") {
        const star = item.expression;
        const relations = star.relation === undefined ? scope.relations : scope.relations.filter((relation) => relation.alias === name(star.relation!));
        if (relations.length === 0) {
          this.#diagnostic("TSQ103", star.relation === undefined ? "SELECT * requires a FROM relation" : `Unknown relation alias ${star.relation.name}`, item.range);
          continue;
        }
        if (star.relation === undefined) for (const [columnName, type] of scope.usingColumns) add({ name: columnName, ...type, range: item.range });
        for (const relation of relations) for (const column of Object.values(relation.table.columns)) {
      ...
      if (output === undefined) {
        if (this.#strict) this.#diagnostic("TSQ104", "Expressions in SELECT require an explicit alias", item.range, "Add AS <name>.");
        continue;
      }
  ...
  #expression(
    ...
    if (expression.kind === "case") {
      ...
      if (expression.elseExpression !== undefined) values.push(this.#expression(expression.elseExpression, scope, ctes));
      return { tsType: union(values.map((value) => value.tsType)), nullable: expression.elseExpression === undefined || values.some((value) => value.nullable) };
    }
    if (expression.kind === "subquery") {
      const result = this.#statement(expression.query, scope, ctes);
      if (result.columns.length !== 1) {
        this.#diagnostic("TSQ216", `Scalar subquery returns ${result.columns.length} columns instead of one`, expression.range);
        return { tsType: "unknown", nullable: true };
      }
  ...
  #binary(expression: Extract<Expression, { readonly kind: "binary" }>, scope: Scope, ctes: ReadonlyMap<string, TableSnapshot>): ResolvedType {
    ...
    if (leftType !== undefined && rightType !== undefined && numericTypes.has(leftType) && numericTypes.has(rightType)) {
      const decimal = leftType === "decimal" || rightType === "decimal" || expression.operator === "/";
      return { tsType: decimal ? this.#policy.decimal : "number", nullable: left.nullable || right.nullable, databaseType: decimal ? "decimal" : "double" };
    }
  ...
  #call(expression: CallExpression, scope: Scope, ctes: ReadonlyMap<string, TableSnapshot>): ResolvedType {
    ...
    if (expression.filter !== undefined) this.#unsupported("MySQL does not support aggregate FILTER", expression.filter.range);
    ...
    if (["SUM", "AVG"].includes(functionName)) return { tsType: this.#policy.decimal, nullable: true, databaseType: "decimal" };
    if (["MIN", "MAX"].includes(functionName)) return { ...(values[0] ?? { tsType: "unknown" }), nullable: true };
    if (functionName === "COALESCE") return { tsType: union(values.map((value) => value.tsType)), nullable: values.every((value) => value.nullable) };
    if (functionName === "IFNULL" || functionName === "NULLIF") return { ...(values[0] ?? { tsType: "unknown" }), nullable: functionName === "NULLIF" || values.every((value) => value.nullable) };
  ...
  #resolveColumn(relationIdentifier: Identifier | undefined, columnIdentifier: Identifier, scope: Scope): ResolvedType {
    ...
    const relations = relationIdentifier === undefined ? scope.relations : scope.relations.filter((relation) => relation.alias === name(relationIdentifier));
    ...
    if (matches.length === 0) {
      ...
      this.#diagnostic(relationIdentifier === undefined ? "TSQ101" : "TSQ103", relationIdentifier === undefined ? `Unknown column ${columnIdentifier.name}` : `Unknown relation or column ${relationIdentifier.name}.${columnIdentifier.name}`, columnIdentifier.range);
      return { tsType: "unknown", nullable: true };
    }
  ...
  #column(table: TableSnapshot, columnName: string): ColumnSnapshot | undefined {
    return Object.entries(table.columns).find(([key, column]) => key.toLowerCase() === columnName.toLowerCase() || column.name.toLowerCase() === columnName.toLowerCase())?.[1];
  }
  ...
  #diagnostic(code: string, message: string, range: SourceRange, suggestion?: string, severity: SqlDiagnostic["severity"] = "error"): void {
    this.#diagnostics.push({ code, message, range, severity, ...(suggestion === undefined ? {} : { suggestion }) });
  }
}

export function resolveMySqlStatement(statement: Statement, schema: SchemaSnapshot, options: ResolveMySqlOptions = {}): ResolvedMySqlQuery {
