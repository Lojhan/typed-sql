...
const schema = {
  ...
  functions: {
    "app.zero()": {
      ...
      name: "zero",
      argumentTypes: [],
      databaseReturnType: "int",
      returnType: "number",
      nullable: false,
    },
    "app.ambiguous(bigint)": {
      schema: "app",
      name: "ambiguous",
...
const codes = (sql: string, source: SchemaSnapshot = schema) =>
  resolveMySqlStatement(mysqlStatement(sql), source).diagnostics.map((value) => value.code);

await describe("MySQL resolver safety matrix", async () => {
  await it("covers CTE, relation, and join scope diagnostics", () => {
    strict.ok(
      codes("WITH x AS (SELECT id FROM users), x AS (SELECT id FROM projects) SELECT id FROM x").includes("TSQ211"),
    );
    ...
    strict.ok(
      codes(
        "SELECT users.id AS user_id, projects.id AS project_id FROM users LEFT JOIN projects ON users.id = projects.owner_id",
      ).length === 0,
    );
    strict.ok(
      codes(
        "SELECT users.id AS user_id, projects.id AS project_id FROM users RIGHT JOIN projects ON users.id = projects.owner_id",
      ).length === 0,
    );
    ...
    const duplicate = {
      ...schema,
      tables: { ...schema.tables, "audit.users": { ...schema.tables.users, schema: "audit" } },
  ...
  await it("covers output naming, permissive mode, and dialect mismatch", () => {
    ...
    const permissive = resolveMySqlStatement(
      mysqlStatement("SELECT id + 1, CAST(id AS SIGNED), COUNT(*), CASE WHEN active THEN 1 END FROM users"),
      schema,
      { strictExpressions: false },
    );
    strict.deepStrictEqual(
      permissive.columns.map((value) => value.name),
      ["id", "count", "case"],
    );
