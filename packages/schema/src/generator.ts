...
export function checkSchemaDrift(generated: GeneratedSchemaSnapshot, current: SchemaSnapshot, policy: unknown = {}): SchemaDriftResult {
...
export async function generateSchemaPackage(schema: SchemaSnapshot, options: GenerateSchemaOptions): Promise<GeneratedSchemaMetadata> {
  ...
  await writeFile(join(options.outDir, "schema.json"), `${JSON.stringify({ ...versionedSchema, metadata }, null, 2)}\n`, "utf8");
  const moduleSource = [
