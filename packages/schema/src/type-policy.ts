import type { SchemaSnapshot, TypePolicy } from "./model.js";
...
export function isKnownPostgresType(databaseType: string, schema?: SchemaSnapshot): boolean {
  ...
  if ([
    ...
    "real", "float4", "double precision", "float8", "boolean", "bool", "text", "varchar",
    "character varying", "char", "uuid", "date", "timestamp", "timestamp without time zone",
    "timestamptz", "timestamp with time zone", "json", "jsonb", "bytea",
  ].includes(type)) return true;
  return schema?.enums?.[type] !== undefined || schema?.domains?.[type] !== undefined;
}

export function mapPostgresType(databaseType: string, policy: TypePolicy, schema?: SchemaSnapshot): string {
  ...
  let mapped: string;
  if (["smallint", "int2", "integer", "int", "int4", "real", "float4", "double precision", "float8"].includes(type)) mapped = "number";
  ...
  else if (["date", "timestamp", "timestamp without time zone", "timestamptz", "timestamp with time zone"].includes(type)) mapped = policy.date;
  else if (["json", "jsonb"].includes(type)) mapped = policy.json;
  else if (type === "bytea") mapped = "Uint8Array";
  else if (schema?.domains?.[unmodifiedType] !== undefined || schema?.domains?.[type] !== undefined) mapped = (schema.domains[unmodifiedType] ?? schema.domains[type]!).tsType;
