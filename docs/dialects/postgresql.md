...
| SQL/JSON query expressions | PostgreSQL 17+ `JSON_EXISTS` owns its context, path, `PASSING`, `FORMAT JSON`, encoding, and `ON ERROR` syntax. It infers `text` context and passing parameters, `jsonpath` path parameters, validates formatted `bytea` and UTF-8 encoding, and models nullable inputs and `UNKNOWN ON ERROR`. Older servers and missing server-version evidence produce version diagnostics. |
