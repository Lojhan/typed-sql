...
| SQL/JSON query expressions | PostgreSQL 17+ `JSON_EXISTS` and `JSON_QUERY` own their context, path, `PASSING`, `FORMAT JSON`, encoding, and error-behavior syntax. `JSON_QUERY` additionally resolves `RETURNING`, wrapper, quote, `ON EMPTY`, and `ON ERROR` clauses, including constant defaults and output nullability. They infer `text` context and passing parameters, `jsonpath` path parameters, and validate formatted `bytea` and UTF-8 encoding. Older servers and missing server-version evidence produce version diagnostics. |
