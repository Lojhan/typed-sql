...
import { analyzeSource, queryAtPosition } from "../packages/ts-bridge/src/index.js";
import {
  NativePreviewTypeScriptBridge,
  TYPESCRIPT_PREVIEW_VERSION,
} from "../packages/ts-bridge/src/native-preview.js";

const testDirectory = dirname(fileURLToPath(import.meta.url));
const workspaceDirectory = resolve(testDirectory, "..");
const fixtureDirectory = join(testDirectory, "fixtures", "success");
...
await describe("TypeScript 7 editor bridge", async () => {
  ...
  const analysis = analyzeSource(source, schema);
